...
function ThemeToggle(): React.ReactElement {
  ...
  return (
    <button
      onClick={toggle}
      className="inline-flex items-center gap-2 rounded-2xl glass px-3 py-2 text-sm hover:shadow"
    >
...
function Experience(): React.ReactElement {
  return (
    <Section
      ...
      title="Experience"
      subtitle="Roles, internships, and notable work"
    >
...
function Projects(): React.ReactElement {
  ...
  const sorted = [...filtered].sort(
    (a, b) => yearVal(b.when) - yearVal(a.when)
  );
  ...
  function ProjectCard({ p }: { p: Project }): React.ReactElement {
    ...
    return (
      <div
        ...
        style={{ ["--accent" as any]: p.accent ?? "#60a5fa" }}
      >
        <div
          ref={ref}
          className="project-tilt card-inner"
          onMouseMove={onMove}
        ...
        >
          ...
          <div className="flex gap-2 flex-wrap mt-3">
            {p.stack.map((s, idx) => (
              <span key={idx} className={`badge ${badgeTone(s)}`}>
                {s}
              </span>
            ))}
          </div>
  ...
  return (
    <Section
      ...
      title="Projects"
      subtitle="Selected work — filter by stack or search"
    >
...
function Footer(): React.ReactElement {
  return (
    <footer
      id="contact"
      className="mt-16 border-t border-zinc-200/60 dark:border-zinc-800/60"
    >
